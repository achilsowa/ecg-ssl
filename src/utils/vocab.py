...
Usage:
    vocab.py [--train-src=<file>] --train-tgt=<file> [--prefix=<string>] [options] VOCAB_FILE

Options:
    -h --help                  Show this screen.
    --train-src=<file>         File of training source sentences: NOT SPECIFIED IN ECGS CASE
    --train-tgt=<file>         File of training target sentences
    --prefix=<string>          File name prefix [default: tgt]
...
from src.utils.nmt import read_corpus, pad_sents
...
class VocabEntry(object):
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device = None) -> torch.Tensor:
    ...
    def decode(self, input_ids: torch.LongTensor) -> List[List[str]]:
        """Reverse of to_input_tensor"""
        sentences = []
        for ids in input_ids.tolist():
            bos = ids.index(self['<s>']) if self['<s>'] in ids else -1
            eos = ids.index(self['</s>']) if self['</s>'] in ids else len(ids)            
            ids = ids[bos+1: eos]
            words = self.indices2words(ids)
...
if __name__ == '__main__':
    ...
    src_sents = get_vocab_list(args['--train-src'], source='src', vocab_size=21000) # EDIT: VOCAB SIZE 8000 => 983
    tgt_sents = [] #get_vocab_list(args['--train-tgt'], source=args['--prefix'], vocab_size=983)      # EDIT: NEW VOCAB SIZE
