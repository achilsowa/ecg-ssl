...
def main(args_eval, resume_preempt=False):
    ...
    if pretrain_folder is None and ckp_fname is not None:
        ckp_fname = os.path.realpath(ckp_fname)
        pretrain_folder = os.path.dirname(ckp_fname)
    ...
    early_stop_patience = args_opt.get('early_stop_patience', 15)
    ...
    folder = os.path.join(pretrain_folder, 'ecg_classification_frozen/')
    ...
    classifier = init_adapter(device, adapter_name, encoder, classifier)
    ...
    if should_train:
        param_groups = get_param_groups(classifier)
        opt = init_opt(
            opt_strategy,
            param_groups,
            wd=wd,
...
def run_one_epoch(
    ...
    if compute_metrics:    
        ...
        scores = calculate_metrics(all_labels, all_outputs, device, y_labels, categories)
    else:
        scores = {}
