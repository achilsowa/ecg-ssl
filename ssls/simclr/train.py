...
def main(args, resume_preempt=False):
    ...
    sobel_derivative = args['data'].get('sobel_derivative', 0.01)
    reverse=args['data'].get('reverse', False)
    invert=args['data'].get('invert', False)
    rand_wanderer=args['data'].get('rand_wanderer', 0.2)
    baseline_wanderer=args['data'].get('baseline_wanderer', 0.2)
    baseline_shift=args['data'].get('baseline_shift', 0.2)
    em_noise=args['data'].get('em_noise', 0.2)
    pl_noise=args['data'].get('pl_noise', 0.2)
    time_out=args['data'].get('time_out', 0.2)
    scale=args['data'].get('scale', 0.2)
    ...
    if isinstance(transform, list):
        ...
        transform = [ContrastiveTransform(tr) for tr in transform]
    else:
        logger.info(f'transforms:\n{transform}')
        transform = ContrastiveTransform(transform)
...
def calculate_metrics(logits, labels):
    ...
    return {"acccuracy": float(acc), "entropy": float(entropy)}
